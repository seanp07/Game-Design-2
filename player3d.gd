...
@onready var camera = $balls/XRCamera3D
...
func _physics_process(delta):
	...
	var rotate_dir = Input.get_vector("rot_left","rot_right","rot_up","rot_dwn" ).normalized()
	if rotate_dir:
...
func _unhandled_input(event):
	if event is InputEventMouseMotion and Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		if first_person:
			self.rotate_y(-event.relative.x * (CAM_SENSITIVITY / 10))
			camera.rotate_x(-event.relative.y * (CAM_SENSITIVITY / 10))
			camera.rotation.x = clamp(camera.rotation.x, deg_to_rad(-40), deg_to_rad (60))
		else:
			self.rotate_y(-event.relative.x * (CAM_SENSITIVITY / 10))
			camera_arm.rotate_x(-event.relative.y * (CAM_SENSITIVITY / 10))
